...
def run(args):
    ...
    if os.path.isfile(f"{representation_folder}/{k}_train.pkl") and os.path.isfile(f"{representation_folder}/{k}_unseen.pkl"):
        print(f'Representations found  after {time.time()-start_time} seconds . . .')
        with open(f"{representation_folder}/{k}_unseen.pkl", 'rb') as f:
            X_unseen,y_unseen = pickle.load(f)

        with open(f"{representation_folder}/{k}_train.pkl", "rb") as f:
            X,y = pickle.load(f)

        print(f'Loaded representations  after {time.time()-start_time} seconds . . .')
        ...
        print(f'The model has been succesfully loaded . . . after {time.time()-start_time} seconds')
        model.to(device)
        model.eval()

        train = pd.read_csv(f"{args['input_path']}/supervised_train.csv")
        test = pd.read_csv(f"{args['input_path']}/unseen.tsv", sep='\t')
        target_level='genus_name'


        X_unseen, y_unseen, orders = data_from_df(test, target_level, model, tokenizer)
        file = open(f"{representation_folder}/{k}_unseen.pkl", "wb")
        pickle.dump((X_unseen,y_unseen), file)
        file.close()

        print(f'Unseen representations saved after {time.time()-start_time} seconds . . .')
        ...
        print(f'Training representations saved after {time.time()-start_time} seconds . . .')
