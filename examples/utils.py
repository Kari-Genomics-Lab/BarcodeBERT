...
import sys
sys.path.append('/home/pmillana/projects/def-khill22/pmillana/DNABERT/src')
...
def predict(args, model, tokenizer, prefix="", n_labels=2):
    # Loop to handle MNLI double evaluation (matched, mis-matched)
    ...
    for pred_task, pred_output_dir in zip(pred_task_names, pred_outputs_dirs):
        pred_dataset = load_and_cache_examples(args, pred_task, tokenizer, evaluate=True, n_labels=n_labels)
        ...
        for batch in tqdm(pred_dataloader, desc="Predicting"):
            ...
            with torch.no_grad():
                ...
                features.append(torch.mean(hidden_state[0], 1,False).cpu().numpy())  #GAP to compute the hidden representations
...
def load_and_cache_examples(args, task, tokenizer, evaluate=False, n_labels=2):
    ...
    if os.path.exists(cached_features_file) and not args.overwrite_cache:
        ...
        label_list = processor.get_labels(num=n_labels)
