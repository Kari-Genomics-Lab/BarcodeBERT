    ...
    def __init__(
        ...
        **filter_args,
        ...
        self.filter_fn = HyenaFilter(
            d_model * (order - 1), order=filter_order, seq_len=l_max, channels=1, dropout=filter_dropout, **filter_args
        )
...
def create_mixer_cls(layer=None, attn_layer_idx=None, attn_cfg=None, layer_idx=None, device=None, dtype=None):
    factory_kwargs = {"device": device, "dtype": dtype}
    if attn_layer_idx is not None and layer_idx in attn_layer_idx:
        ...
        mixer_cls = partial(
            mha_cls, causal=causal, layer_idx=layer_idx, **(attn_cfg if attn_cfg is not None else {}), **factory_kwargs
        )
    else:
...
def create_mlp_cls(d_model, d_inner=None, device=None, dtype=None):
    ...
    mlp_cls = partial(Mlp, hidden_features=inner_dim, activation=partial(F.gelu, approximate="tanh"), **factory_kwargs)
...
def create_block(
    ...
    mixer_cls = create_mixer_cls(
        layer=layer, attn_layer_idx=attn_layer_idx, attn_cfg=attn_cfg, layer_idx=layer_idx, **factory_kwargs
    )
        ...
        for name, p in module.named_parameters():
            ...
            # If using GLU activation for now, we scale the std by 2
                ...
                if not glu_act:
                    ...
                    # on average.
                    nn.init.normal_(
                        p[: out_features // 2], mean=0.0, std=initializer_range / math.sqrt(2 * n_layer) * 2
                    )
...
class SequenceDecoder(nn.Module):
    ...
    def forward(self, x, state=None, lengths=None, l_output=None):
        ...
        if self.use_lengths:
            ...
            x = torch.stack(
                [restrict(out[..., :length, :]) for out, length in zip(torch.unbind(x, dim=0), lengths)],
                dim=0,
            )
        else:
            x = restrict(x)
...
class HyenaDNAPreTrainedModel(PreTrainedModel):
    ...
    def from_pretrained(
        ...
        loaded_ckpt = torch.load(
            os.path.join(pretrained_model_name_or_path, "weights.ckpt"), map_location=torch.device(device)
        )
